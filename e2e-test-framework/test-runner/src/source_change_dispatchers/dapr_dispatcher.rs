...
use crate::{runner::config::DaprSourceChangeDispatcherConfig, script_source::SourceChangeEvent};
...
impl DaprSourceChangeDispatcherSettings {
    pub fn try_from_config(config: &DaprSourceChangeDispatcherConfig, source_id: String) -> anyhow::Result<Self> {
        Ok(Self {
            ...
            pubsub_name: config.pubsub_name.clone().unwrap_or("rg-pubsub".to_string()),
