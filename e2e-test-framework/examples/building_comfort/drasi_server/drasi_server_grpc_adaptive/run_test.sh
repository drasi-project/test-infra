...
echo -e "${GREEN}\nRunning E2E Test Service with Adaptive gRPC Batching...${RESET}"
echo -e "${YELLOW}Configuration:${RESET}"
echo -e "  - Adaptive batching: ENABLED"
echo -e "  - Dynamic batch sizes: 1-2000 messages"
echo -e "  - Adaptive wait times: 0.1ms-100ms"
...
RUST_LOG='info,test_run_host::utils::adaptive_batcher=error,drasi_core::query::continuous_query=error,drasi_core::path_solver=error' \
    ...
    --config examples/building_comfort/drasi_server_grpc_adaptive/config.json