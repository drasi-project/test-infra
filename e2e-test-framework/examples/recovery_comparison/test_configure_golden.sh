...
jq -e '.data_store.test_repos[0].local_tests[0].completion_handlers |
    ...
    .[1].enforce == false and (.[1] | has("policy") | not) and
    (.[1].queries | length == 2)' "$config" >/dev/null
...
jq -e '.data_store.test_repos[0].local_tests[0].completion_handlers[1] |
    (has("policy") | not) and .enforce == false and
...
for variant in config config.grpc_adaptive config.http config.http_adaptive; do
    ...
    jq -e '(has("policy") | not) and .enforce == false' "$directory/handler.json" >/dev/null
    case "$variant" in
        config.http*)
...
printf 'PASS: current golden, all four transports, unchanged snapshots, no policy override, and workload guards. Artifacts: %s\n' "$directory"