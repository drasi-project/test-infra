...
impl DataCollectionSource {
    pub async fn new(
        ...
        let bootstrap_data_recorder = match &config.bootstrap_data_recorder {
            Some(config) => {
                Some(BootstrapDataRecorder::new(config, source_id.clone(), storage.clone()).await?)
            }
            None => None,
        };
        ...
        let source_change_recorder = match &config.source_change_recorder {
            Some(config) => {
                Some(SourceChangeRecorder::new(config, source_id.clone(), storage.clone()).await?)
            }
            None => None,
        };
    ...
    pub async fn start_source_change_recorder(
        &mut self,
    ) -> anyhow::Result<SourceChangeRecorderMessageResponse> {
        log::debug!(
            "Starting source change recorder for source {}",
            self.source_id
        );
        ...
        if let Some(source_change_recorder) = &mut self.source_change_recorder {
            ...
            anyhow::bail!(
                "No source change recorder configured for source {}",
                self.source_id
            );
        }
    }

    pub async fn pause_source_change_recorder(
        &mut self,
    ) -> anyhow::Result<SourceChangeRecorderMessageResponse> {
        log::debug!(
            "Pausing source change recorder for source {}",
            self.source_id
        );
    ...
    pub async fn stop_source_change_recorder(
        &mut self,
    ) -> anyhow::Result<SourceChangeRecorderMessageResponse> {
        log::debug!(
            "Stopping source change recorder for source {}",
            self.source_id
        );
