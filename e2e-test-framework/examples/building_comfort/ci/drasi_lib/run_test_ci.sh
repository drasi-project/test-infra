...
#
# Responsibilities:
#   1. Patch the example config so the run is CI-safe (keep artifacts on
#      shutdown, redirect the data-store path into $WORK_DIR, enforce that
#      the model source has a `seed`).
#   2. Start the test-service as a background process.
#   3. Wait for the completion-tracker signal in the test-service log.
#   4. Snapshot each reaction's final state and the test-run status via
#      the REST API, write log groups + a markdown summary into
#      $GITHUB_STEP_SUMMARY.
#
...
TEST_REACTION_IDS="${TEST_REACTION_IDS:-building-comfort}"
