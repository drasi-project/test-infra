# Building Comfort &mdash; External Drasi Server (HTTP webhook, adaptive batching)

End-to-end test that drives an **external Drasi Server** over HTTP webhooks
with **adaptive batching** enabled on the source dispatcher. This is the CI
variant of
[`local/drasi_server_http_adaptive`](../../local/drasi_server_http_adaptive)
and mirrors [`ci/drasi_server_http`](../drasi_server_http); the only
difference is that the test framework dispatches change events with
adaptive HTTP batching (`batch_events: true`, `adaptive_enabled: true`,
`batch_size: 1000`, `batch_timeout_ms: 50`). Adaptive batching is a
transport optimization, so results &mdash; and therefore the determinism
baselines &mdash; are identical to the non-adaptive `drasi_server_http`
variant.

It is **not** part of the scheduled / default CI set; run it manually via
the `.github/workflows/e2e-building-comfort.yml` `workflow_dispatch`
**variants** input by including `drasi_server_http_adaptive` in the
