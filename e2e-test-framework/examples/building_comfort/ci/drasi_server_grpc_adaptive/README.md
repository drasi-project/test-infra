# Building Comfort &mdash; External Drasi Server (gRPC, adaptive batching, CI)

End-to-end test that drives an **external Drasi Server** over gRPC with
**adaptive batching** enabled on the source dispatcher. This is the CI
variant of
[`local/drasi_server_grpc_adaptive`](../../local/drasi_server_grpc_adaptive)
and mirrors [`ci/drasi_server_grpc`](../drasi_server_grpc); the only
difference is that the test framework dispatches change events with
adaptive gRPC batching (`adaptive_enabled: true`, `batch_size: 1000`,
`batch_timeout_ms: 50`). Adaptive batching is a transport optimization,
so results &mdash; and therefore the determinism baselines &mdash; are
identical to the non-adaptive `drasi_server_grpc` variant.
...
The workflow at `.github/workflows/e2e-building-comfort.yml` runs the
building_comfort variants via a matrix. This adaptive variant is **not**
part of the scheduled / default set; run it manually via the
`workflow_dispatch` **variants** input by including
`drasi_server_grpc_adaptive` in the comma-separated list.

## Run it locally (Apple Silicon)

```bash
cd e2e-test-framework/examples/building_comfort/ci/drasi_server_grpc_adaptive
