...
use super::{CommonTestRepoConfig, HuggingFaceTestRepoConfig, RemoteTestRepoClient};
...
impl HuggingFaceTestRepoClient {
    ...
    pub async fn new(
        ...
        log::debug!(
            "Creating HuggingFaceTestRepoClient from common_config:{common_config:?} and unique_config:{unique_config:?}, "
        );
...
async fn download_huggingface_repo_file(
    ...
    let url = format!(
        "https://huggingface.co/datasets/{organization}/{dataset}/resolve/{revision}/{remote_path}"
    );
    ...
    loop {
        ...
        if response.status() == reqwest::StatusCode::TOO_MANY_REQUESTS && attempt < MAX_RETRIES {
            ...
            log::warn!(
                "Rate limited downloading {remote_path}. Retrying in {backoff_ms}ms (attempt {attempt}/{MAX_RETRIES})"
            );
...
fn download_huggingface_repo_folder(
    ...
    async move {
        log::debug!("Downloading folder {remote_repo_folder} to {local_repo_folder:?}");
