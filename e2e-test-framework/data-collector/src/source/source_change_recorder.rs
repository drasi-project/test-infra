...
use test_data_store::{
    data_collection_storage::DataCollectionSourceStorage, scripts::ChangeScriptRecord,
};

use crate::{
    config::{SourceChangeQueueReaderConfig, SourceChangeRecorderConfig},
    source::change_queue_readers::{
        get_source_change_queue_reader, none_change_queue_reader::NoneSourceChangeQueueReader,
        SourceChangeQueueReader, SourceChangeQueueReaderMessage,
    },
};
...
impl SourceChangeRecorderSettings {
    pub fn new(
        config: &SourceChangeRecorderConfig,
        source_id: String,
...
impl SourceChangeRecorder {
    pub async fn new(
        ...
        let recorder_thread_handle =
            tokio::spawn(recorder_thread(settings.clone(), player_rx_channel));
    ...
    pub async fn get_state(&self) -> anyhow::Result<SourceChangeRecorderMessageResponse> {
        self.send_command(SourceChangeRecorderCommand::GetState)
            .await
    }
    ...
    async fn send_command(
        &self,
        command: SourceChangeRecorderCommand,
    ) -> anyhow::Result<SourceChangeRecorderMessageResponse> {
        let (response_tx, response_rx) = oneshot::channel();

        let r = self
            .recorder_tx_channel
            .send(SourceChangeRecorderMessage {
                command,
...
pub async fn recorder_thread(
    ...
    let mut recorder_state = SourceChangeRecorderState {
        ...
        error_message: None,
        last_record: None,
        start_record_time: SystemTime::now()
    ...
    let change_queue_reader: Box<dyn SourceChangeQueueReader + Send + Sync> =
        match get_source_change_queue_reader(
            settings.source_change_queue_reader_config,
            &settings.source_id,
        )
        .await
        {
...
fn transition_from_paused_state(
    command: &SourceChangeRecorderCommand,
    recorder_state: &mut SourceChangeRecorderState,
) -> anyhow::Result<()> {
    log::debug!(
        "Transitioning from {:?} state via command: {:?}",
        recorder_state.status,
        command
    );
    ...
    match command {
        ...
        SourceChangeRecorderCommand::Pause => Err(SourceChangeRecorderError::AlreadyPaused.into()),
        SourceChangeRecorderCommand::Stop => {
            recorder_state.status = SourceChangeRecorderStatus::Stopped;
            Ok(())
        }
        SourceChangeRecorderCommand::GetState => Ok(()),
    }
}

fn transition_from_running_state(
    command: &SourceChangeRecorderCommand,
    recorder_state: &mut SourceChangeRecorderState,
) -> anyhow::Result<()> {
    log::debug!(
    ...
    match command {
        SourceChangeRecorderCommand::Start => Err(SourceChangeRecorderError::AlreadyRunning.into()),
...
fn transition_from_stopped_state(
    command: &SourceChangeRecorderCommand,
    recorder_state: &mut SourceChangeRecorderState,
) -> anyhow::Result<()> {
    log::debug!(
...
fn transition_from_error_state(
    command: &SourceChangeRecorderCommand,
    recorder_state: &mut SourceChangeRecorderState,
) -> anyhow::Result<()> {
    log::debug!(
